      ...
      child: FutureBuilder<WebMetadata>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            widget.onGetData.call(
              VLinkPreviewData(
                link: data.url,
                title: data.title,
                description: data.description ?? data.url,
                image: data.images.isEmpty ? data.favicons.first : data.images.first,
              ),
            );
            ...
            return Padding(
              ...
              child: ListTile(
                leading: CachedNetworkImage(
                  imageUrl: data.images.isEmpty ? data.favicons.first : data.images.first,
                  width: 70,
                  fit: BoxFit.cover,
                ),
                title: data.title.h6,
                subtitle:
                    (data.description ?? data.url).h6.color(Colors.grey).size(12).maxLine(3),
              ),
            );
          }
        },
