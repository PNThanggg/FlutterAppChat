...
class VPlatformFile {
  ...
  factory VPlatformFile.fromMap(Map<String, dynamic> map) {
    ...
    final networkUrl = (map['networkUrl'] as String?) ?? (map['url'] as String?);
    ...
    if (filePath == null && bytes == null && networkUrl == null) {
      throw ArgumentError("At least one of 'filePath', 'bytes', or 'networkUrl' must not be null. Map: $map");
    }
    ...
    final file = VPlatformFile._(
      ...
      fileHash: (map['fileHash'] as String?) ?? basenameWithoutExtension(name).replaceAll(" ", "-"),
    );
