...
class VPlatformFile {
  ...
  String? get url {
    ...
    if (VPlatformFileUtils.baseMediaUrl == null) {
      return baseUrl;
    }
  ...
  @override
  String toString() {
    return 'PlatformFileSource{name: $name, url:$url _baseUrl:$baseUrl filePath: $fileLocalPath, mimeType: $mimeType, '
        'assetsPath: $assetsPath, size: $fileSize bytes ${bytes?.length}}';
  }
  ...
  factory VPlatformFile.fromMap(Map<String, dynamic> map) {
    ...
    return VPlatformFile._(
      ...
      fileHash: (map['fileHash'] as String?) ?? basenameWithoutExtension(map['name'] as String).replaceAll(" ", "-"),
    );
  }
