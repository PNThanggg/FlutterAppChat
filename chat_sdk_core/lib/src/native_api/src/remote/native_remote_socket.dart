...
class NativeRemoteSocketIo {
  ...
  Completer<void> get socketCompleter => SocketController.instance.socketCompleter;
  ...
  void emitGetMyOnline(List<Map<String, dynamic>> ids) => SocketController.instance.emitGetMyOnline(
        jsonEncode(ids),
      );

  /// Emits an event to update the typing status of a room with the given [model].
  void emitUpdateRoomStatus(VSocketRoomTypingModel model) => SocketController.instance.emitUpdateRoomStatus(
        jsonEncode(
          model.toMap(),
        ),
      );
  ...
  void emitDeliverRoomMessages(String roomId) => SocketController.instance.emitDeliverRoomMessages(
        jsonEncode(
          {
            "roomId": roomId,
          },
  ...
  void emitSeenRoomMessages(String roomId) => SocketController.instance.emitSeenRoomMessages(
        jsonEncode(
          {
            "roomId": roomId,
          },
