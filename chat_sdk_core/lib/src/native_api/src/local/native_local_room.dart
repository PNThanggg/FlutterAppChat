...
class NativeLocalRoom {
  ...
  Future<VRoom?> getOneWithLastMessageByRoomId(String id) => _roomRepo.getOneWithLastMessageByRoomId(id);
  ...
  Future<int> cacheRooms(
    ...
    await _localMessage.cacheRoomMessages(rooms.map((e) => e.lastMessage).toList());
  ...
  Future<int> updateRoomTyping(VSocketRoomTypingModel typing) async {
    final event = VUpdateRoomTypingEvent(roomId: typing.roomId, typingModel: typing);
    _emitter.fire(event);
    return 1;
  }
  ...
  Future<VRoom?> getRoomByPeerId(String peerId) async => _roomRepo.getOneByPeerId(peerId);
