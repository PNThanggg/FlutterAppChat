...
class VChannelApiService {
  ...
  Future<VPaginationModel<VRoom>> getRooms(VRoomsDto dto) async {
    ...
    return VPaginationModel<VRoom>.fromCustomMap(
      values: (data['docs'] as List).map((e) => VRoom.fromMap(e as Map<String, dynamic>)).toList(),
      map: data,
    );
  }

  Future<VPaginationModel<VRoom>> getWorkspace(VRoomsDto dto) async {
    final res = await _channelApiService!.getWorkspace(dto.toMap());
    throwIfNotSuccess(res);
    final data = extractDataFromResponse(res);
    return VPaginationModel<VRoom>.fromCustomMap(
      values: (data['docs'] as List).map((e) => VRoom.fromMap(e as Map<String, dynamic>)).toList(),
  ...
  Future<VPaginationModel<VRoom>> getList(VRoomsDto dto) async {
    final res = await _channelApiService!.getList(dto.toMap());
    throwIfNotSuccess(res);
    final data = extractDataFromResponse(res);
    return VPaginationModel<VRoom>.fromCustomMap(
      values: (data['docs'] as List).map((e) => VRoom.fromMap(e as Map<String, dynamic>)).toList(),
  ...
  Future<List<VMentionModel>> searchToMention(
    ...
    final res = await _channelApiService!.getGroupMembers(roomId, filter.toMap());
    throwIfNotSuccess(res);
  ...
  Future<List<VMessageStatusModel>> getMessageStatusForGroup({
    ...
    return resList.map((e) => VMessageStatusModel.fromMap(e as Map<String, dynamic>)).toList();
  }
  ...
  Future<List<VGroupMember>> getGroupMembers(
    ...
    return data.map((e) => VGroupMember.fromMap(e as Map<String, dynamic>)).toList();
  }
  ...
  Future<bool> addParticipantsToGroup({
    ...
    final res = await _channelApiService!.addParticipantsToGroup(roomId, {"ids": ids});
    throwIfNotSuccess(res);
    return true;
  }
