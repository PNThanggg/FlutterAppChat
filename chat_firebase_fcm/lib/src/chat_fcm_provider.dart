...
class ChatFcmProver extends VChatPushProviderBase {
  ...
  Future<bool> init() async {
    try {
      ...
      if (kDebugMode) {
        print(err.toString() == "Google Play services missing or without correct permission.");
      }
  ...
  void _initStreams() {
    ...
    _onMsgClicked = FirebaseMessaging.onMessageOpenedApp.listen((remoteMsg) async {
      final String? fromVChat = remoteMsg.data['fromVChat'];
      final String? message = remoteMsg.data['vMessage'];
      ...
      if (fromVChat != null && message != null) {
        ...
        if (room == null) return;
        _vEventBusSingleton.fire(VOnNotificationsClickedEvent(message: msg, room: room));
      }
    });
  }
