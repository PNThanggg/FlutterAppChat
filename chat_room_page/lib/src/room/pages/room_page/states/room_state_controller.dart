...
class RoomStateController extends ValueNotifier<VPaginationModel<VRoom>> {
  ...
  final workspaceStateStream = StreamController<VRoom>.broadcast(sync: true);

  List<VRoom> get stateRooms => value.data;

  void updateCacheStateForChatRooms(VPaginationModel<VRoom> paginationModel) {
    final Map<String, VRoom> stateRoomMap = Map.fromEntries(
      value.data.map(
        (vRoom) => MapEntry(vRoom.id, vRoom),
      ),
    );
  ...
  void close() {
    ...
    workspaceStateStream.close();
  }

  VRoom? roomById(String roomId) => stateRooms.firstWhereOrNull(
        (e) => e.id == roomId,
      );
  ...
  void updateOnline(String roomId) {
    ...
    if (room != null) {
      ...
      if (room.tgGroupId != null) {
        workspaceStateStream.sink.add(room);
      } else {
  ...
  void setRoomSelected(String roomId) {
    ...
    for (int i = 0; i < value.data.length; i++) {
      if (value.data[i].isSelected) {
        ...
        workspaceStateStream.sink.add(value.data[i]);
