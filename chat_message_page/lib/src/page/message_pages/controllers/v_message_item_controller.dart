...
class VMessageItemController {
  ...
  Function? _starSuccessCallback;
  Function? _unStarSuccessCallback;

  void setStarSuccessCallback(Function action) {
    _starSuccessCallback = action;
  }

  void setUnStarSuccessCallback(Function action) {
    _unStarSuccessCallback = action;
  }
  ...
  ModelSheetItem<VMessageItemClickRes> _translateItem() {
    return ModelSheetItem(
      id: VMessageItemClickRes.translate,
      title: "Translate",
      iconData: const Icon(Icons.translate),
    );
  }
  ...
  void onMessageItemLongPress(
    ...
    if (message.allDeletedAt != null || (message.isOneSeen && !message.isMeSender)) {
      items.clear();
      //solution
    ...
    final ModelSheetItem<dynamic>? res = await VAppAlert.showModalSheetWithActions(
      content: items,
    ...
    switch (res.id as VMessageItemClickRes) {
      ...
      case VMessageItemClickRes.share:
        ...
        break;
      case VMessageItemClickRes.translate:
        _handleTranslate(message);
        break;
  ...
  void _handleForward(VBaseMessage baseMessage) async {
    final List<String> ids = await VChatController.I.vNavigator.roomNavigator.toForwardPage(
      context,
      baseMessage.roomId,
    );
    if (ids.isNotEmpty) {
  ...
  void _handleTranslate(VBaseMessage message) async {
    VAppAlert.showTranslateResult(
      context: context,
      title: "Translate",
  ...
  void _handleDelete(VBaseMessage message) async {
    ...
    if (message.isMeSender && !message.isAllDeleted && message.emitStatus.isServerConfirm) {
