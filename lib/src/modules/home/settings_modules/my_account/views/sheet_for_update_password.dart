...
class UpdatePasswordSheetWidget extends StatefulWidget {
  ...
  @override
  State<UpdatePasswordSheetWidget> createState() => _UpdatePasswordSheetWidgetState();
}
...
class _UpdatePasswordSheetWidgetState extends State<UpdatePasswordSheetWidget> {
  ...
  Widget build(BuildContext context) {
    return CupertinoPageScaffold(
      ...
      navigationBar: CupertinoNavigationBar(
        ...
        middle: Text(S.of(context).updateYourPassword),
      ),
      child: SafeArea(
        child: Padding(
  ...
  void onRequest(BuildContext context) async {
    await vSafeApiCall(
      ...
      onSuccess: (response) {
        VAppAlert.showSuccessSnackBar(message: S.of(context).passwordHasBeenChanged, context: context);
        widget.onCloseSheet();
      },
      onError: (exception, trace) {
        VAppAlert.showOkAlertDialog(content: exception.toString(), context: context, title: S.of(context).error);
      },
    );
    setState(() {
      loading = false;
...
//<editor-fold desc="Data Methods">
  ...
  bool get isValid {
    if (oldPassword.isEmpty || newPassword.isEmpty || newConfPassword.isEmpty || newPassword != newConfPassword) {
      return false;
    }
    return true;
  }
