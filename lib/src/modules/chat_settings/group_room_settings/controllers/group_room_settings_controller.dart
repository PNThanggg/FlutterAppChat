...
import '../../chat_star_messages/views/chat_star_messages_page.dart';
import '../mobile/sheet_for_add_members_to_group.dart';
import '../states/group_room_setting_state.dart';

class GroupRoomSettingsController extends SLoadingController<GroupRoomSettingState> {
  ...
  void onChangeGroupDescriptionClicked(BuildContext context) async {
    ...
    if (newTitle != settingsModel.title) {
      await vSafeApiCall<String>(
        request: () async {
          await VChatController.I.roomApi.updateGroupDescription(roomId: roomId, description: newTitle);
          return newTitle;
        },
        onSuccess: (response) {
  ...
  void toUpdateNickName(BuildContext context) async {
    ...
    if (text != null) {
      await VChatController.I.nativeApi.remote.room.updateRoomNickName(roomId, text);
  ...
  void openStarredMessages(BuildContext context) {
    context.toPage(
      ChatStarMessagesPage(roomId: roomId),
    );
  }
  ...
  Future openEditImage(BuildContext context) async {
    ...
    await vSafeApiCall<String>(
      request: () async {
        ...
        await VChatController.I.nativeApi.local.room.updateRoomImage(VUpdateRoomImageEvent(roomId: roomId, image: url));
        return url;
      },
      onSuccess: (response) {
      ...
      onError: (exception, trace) {
        VAppAlert.showErrorSnackBar(message: S.of(context).error, context: context);
      },
    );
  }
