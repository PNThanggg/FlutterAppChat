...
class SingleRoomSettingsController extends SLoadingController<SingleRoomSettingState> {
  ...
  bool get isCallAllowed => VPlatforms.isMobile ? VAppConfigController.appConfig.allowCall : false;
  ...
  Future onBlockUser(BuildContext context) async {
    ...
    final res = await VAppAlert.showAskYesNoDialog(
      ...
      content: "${S.of(context).areYouSureToBlock} ${value.data.settingsModel.title}",
    );
    if (res == 1) {
      await vSafeApiCall(
        onLoading: () {
  ...
  Future _onUnBlock(BuildContext context) async {
    final res = await VAppAlert.showAskYesNoDialog(
      ...
      content: "${S.of(context).areYouSureToUnBlock} ${value.data.settingsModel.title}",
    );
    if (res == 1) {
      await vSafeApiCall(
        onLoading: () {
